...
public class AtprotoData
{
    ...
    public static (string did, string signingKey, string handle, string? pds) ParseToAtprotoDocument(DidDoc doc)
    {
        var did = GetDid(doc);
        var key = GetKey(doc);
    ...
    public static string GetDid(DidDoc doc)
    {
        return doc.Id;
    }

    public static string GetHandle(DidDoc doc)
    {
    ...
    public static string? GetPds(DidDoc doc)
    {
        return GetServiceEndpoint(doc, ("#atproto_pds", "AtprotoPersonalDataServer"));
    }

    public static string? GetServiceEndpoint(DidDoc doc, (string id, string type) opts)
    {
        var did = GetDid(doc);
    ...
    public static string GetKey(DidDoc doc)
    {
        var did = GetDid(doc);
